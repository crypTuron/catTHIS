...
void PrivacyDialog::on_pushButtonMintzCATS_clicked()
{
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zCATS, true));
        if (!ctx.isValid()) {
...
void PrivacyDialog::on_pushButtonSpendzCATS_clicked()
{
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zCATS, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
...
void PrivacyDialog::sendzCATS()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Cats Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CATS </b>will be added to the standard transaction fees!<br />";
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zCATS was spent successfully update the addressbook with the label
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zCATS "));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zCATS </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zCATS </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zCATS </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    //bool fButtonsEnabled =  ui->pushButtonMintzCATS->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        ...
        // Mint zCATS
        //ui->pushButtonMintzCATS->setEnabled(true);
        //ui->pushButtonMintzCATS->setToolTip(tr("PrivacyDialog", "Enter an amount of CATS to convert to zCATS", 0));

        // Spend zCATS
        ui->pushButtonSpendzCATS->setEnabled(true);
        ui->pushButtonSpendzCATS->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
