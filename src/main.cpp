...
bool ContextualCheckZerocoinSpendNoSerialCheck(const CTransaction& tx, const libzerocoin::CoinSpend* spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (pindex->nHeight >= Params().Zerocoin_Block_Public_Spend_Enabled()) {
        ...
        if (v1Serial) {
            return error("%s : zCATS v1 serial spend not spendable, serial %s, tx %s\n", __func__,
...

    {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zCATS spend tx %s already in block %d",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 241000000 * COIN;
    } else if (nHeight <= Params().LAST_POW_BLOCK() && nHeight > 1) { //end PoW
        nSubsidy = 10 * COIN;
    } else if (nHeight <= 5000000 && nHeight > Params().LAST_POW_BLOCK()) { //Start PoS
        nSubsidy = 17 * COIN;
    } else {
        nSubsidy = 17 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZPIVStake)
{
    ...
    } else if (nHeight <= 2000 && nHeight > 165) {
        ret = blockValue / 100 * 80; //80%
    } else if (nHeight > 2000) {
        ret = blockValue / 100 * 94; //94%
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from CATS
...
void static UpdateTip(CBlockIndex* pindexNew)
{
     ...
     *
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert CATS to zPIV
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Cats Specific reference to the block with the wrong threshold was used.
        if ((block.nTime == (uint32_t) Params().CatsBadBlockTime()) && (block.nBits == (uint32_t) Params().CatsBadBlocknBits())) {
            // accept CATS block minted with incorrect proof of work threshold
            return true;
        }
